...
export const pitch = Array(3).fill().reduce((pitch, _, index) => [...pitch, ...[32.7, 36.7, 41.2, 49, 55].map(v => v * (Math.pow(2, index + 2)))], []);

const sounds = [
  [,0,1e3,.01,.08,.09,4,,-20.1,8.4,,,,1,,,,.51,.02], //slash
  [1.5,.5,90,,,1,1,1.91,,,,,,5,50,3], //damage
  [,0,262.6,,,.3,,1.2,.2,.1,-50,.02,,-0.1,.1,.1,.17],
  [,,1774,.03,,.06,4,.01,,,-697,.03,,5,6,,.13,.18,.01,.5], // jump
];

const soundtime = [];

export function playSound(index, interval, pitchIndex) {
  const now = Date.now();
  if(!interval || (!soundtime[index] || now - soundtime[index] > 200)) {
    const sound = sounds[index].slice();
    if(pitchIndex) sound[2] = pitch[pitchIndex % pitch.length];
    zzfx(...sound);
    soundtime[index] = now;
  }
}
